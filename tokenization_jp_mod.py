...
class BertJapaneseTokenizer(BertTokenizer):
        ...
        if do_word_tokenize:
            if word_tokenizer_type == "basic":
                ...
                self.word_tokenizer = SentencePiecepTokenizer(
                    model_file=str(path_vocab.parent / path_vocab.stem) + '.model',
                    do_lower_case=do_lower_case, never_split=never_split, **(mecab_kwargs or {})
                )
            else:
    ...
    def __setstate__(self, state):
        ...
        elif self.word_tokenizer_type == "sp":
            path_vocab = Path(vocab_file)
            self.word_tokenizer = SentencePiecepTokenizer(
                    model_file=str(path_vocab.parent / path_vocab.stem) + '.model',
...
class MecabTokenizer:
    ...
    def __init__(
        ...
        mecab_dic_path: Optional[str] = None,
        mecab_userdic_path: Optional[str] = None,
        ...
        if dic_dir is not None:
            mecabrc = os.path.join(dic_dir, "mecabrc")
            dic_option = '-d "{}"'.format(dic_dir)
        elif mecab_dic_path is not None:
            mecabrc = os.path.join(mecab_dic_path, "mecabrc")
            dic_option = '-d "{}"'.format(mecab_dic_path)
        if mecab_userdic_path is not None:
            dic_option += ' -u "{}"'.format(mecab_userdic_path)
           
        rc_path = Path(mecabrc)
        if not rc_path.exists():
            mecabrc = './mecabrc.tmp'
            Path(mecabrc).write_text('; This file can be empty but it needs to exist for MeCab.')
            
        dic_option += ' -r "{}"'.format(mecabrc)
            
        mecab_option = dic_option + mecab_option
...
# ADD
class SentencePiecepTokenizer:
    """Runs basic tokenization with SentencePiece."""
